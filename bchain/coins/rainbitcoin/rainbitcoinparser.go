...
func init() {
	// rainbitcoin mainnet Address encoding magics
	...
	// rainbitcoin testnet Address encoding magics
...
// rainbitcoinParser handle
type rainbitcoinParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewrainbitcoinParser returns new rainbitcoinParser instance
func NewrainbitcoinParser(params *chaincfg.Params, c *btc.Configuration) *rainbitcoinParser {
	p := &rainbitcoinParser{
...
// GetChainParams contains network parameters for the main rainbitcoin network
...
func (p *rainbitcoinParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
...
func (p *rainbitcoinParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *rainbitcoinParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *rainbitcoinParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *rainbitcoinParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *rainbitcoinParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *rainbitcoinParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *rainbitcoinParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
