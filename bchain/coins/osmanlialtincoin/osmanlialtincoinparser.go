...
// OsmanlialtincoinParser handle
type OsmanlialtincoinParser struct {
	*btc.BitcoinLikeParser
	baseparser *bchain.BaseParser
}

// NewOsmanlialtincoinParser returns new OsmanlialtincoinParser instance
func NewOsmanlialtincoinParser(params *chaincfg.Params, c *btc.Configuration) *OsmanlialtincoinParser {
	return &OsmanlialtincoinParser{
		BitcoinLikeParser: btc.NewBitcoinLikeParser(params, c),
		baseparser:        &bchain.BaseParser{},
	}
}

// GetChainParams contains network parameters for the main Osmanlialtincoin network,
// and the test Osmanlialtincoin network
func GetChainParams(chain string) *chaincfg.Params {
	// register bitcoin parameters in addition to Osmanlialtincoin parameters
	// Osmanlialtincoin has dual standard of addresses and we want to be able to
...
// fallbackTx is used to handle situation when Osmanlialtincoin mainnet returns
...
func (p *OsmanlialtincoinParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *OsmanlialtincoinParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *OsmanlialtincoinParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
